...
produtos = [
    ...
    {'codigo': 17,
        'nome': 'Set Of 2 Dvd9 Self Learning Cbts (Hindi)', 'valor': 5.00},
    ...
    {'codigo': 68,
        'nome': 'Smart Shine Dishwash Bar 200gm (Set Of 4)', 'valor': 0.22},
    {'codigo': 69,
        'nome': 'Smart Wash Detergent Bar 200gm (Set Of 4)', 'valor': 0.22},
    ...
    {'codigo': 80,
        'nome': 'Saathi Pair Of Watches (Gold Plated)', 'valor': 20.00},
...
def imprimir_cabecalho(erro_para_imprimir):
    ...
    pr.retangulo('{reprograma}\nProjeto Guiado I\nTerminal de vendas',
                 tamanho=120, cor_barra='magenta', cor_texto='azul')
    pr.separador(120, cor_texto='azul')
    if (erro_para_imprimir != ''):
        pr.imprimir(erro_para_imprimir, cor_texto='vermelho negrito',
                    tamanho=120, alinhar='centro')
        pr.separador(120, cor_texto='azul')


def imprimir_rodape():
    pr.imprimir('[S] Sair', '[H] Ajuda', tamanho=110,
                alinhar='fim', caracter='─', end='┤')
    return input().lower()
...
def imprimir_compra_fechada(compra, valor_total, descontos):
    ...
    total = 0
    for item in compra:
        pr.imprimir(str(item['codigo']), tamanho=6,
                    end='│', alinhar='fim', caracter='0')
        pr.imprimir(item['nome'], tamanho=83, end='│', caracter='.')
        pr.imprimir(str(item['quantidade']), tamanho=3, end='│', alinhar='fim')
        pr.imprimir('R$', str(round(item['valor'], 2)),
                    tamanho=12, end='│', alinhar='fim')
        pr.imprimir('R$', str(
            round(item['valor'] * item['quantidade'], 2)), tamanho=12, alinhar='fim')
    ...
    for desconto in descontos:
        ...
        pr.imprimir('-R$', str(round(desconto['valor'], 2)),
                    tamanho=12, alinhar='fim', cor_texto='vermelho negrito')
    pr.imprimir('Total a pagar', tamanho=107, end='│', alinhar='fim')
    pr.imprimir('R$', str(round(valor_total, 2)), tamanho=12,
                alinhar='fim', cor_texto='verde negrito')
...
def imprimir_nova_compra(compra, quantidade):
    if (len(compra) > 0):
        ...
        for item in compra:
            ...
            pr.imprimir(item['nome'], tamanho=83, end='│', caracter='.')
            pr.imprimir(str(item['quantidade']),
                        tamanho=3, end='│', alinhar='fim')
            pr.imprimir(
                'R$', str(round(item['valor'], 2)), tamanho=12, end='│', alinhar='fim')
            pr.imprimir('R$', str(
                round(item['valor'] * item['quantidade'], 2)), tamanho=12, alinhar='fim')
        ...
        pr.pular_linha(quantidade=2)
        pr.imprimir('Essa compra não tem itens ainda',
                    tamanho=120, alinhar='centro')
        pr.pular_linha(quantidade=2)
...
def imprimir_caixa_encerrada(compras):
    ...
    for compra in compras:
        pr.imprimir(compra['data'].strftime(
            "%d/%m/%Y %H:%M:%S"), tamanho=92, end='│', alinhar='fim')
        pr.imprimir(str(len(compra['compra'])),
                    tamanho=6, end='│', alinhar='centro')
        pr.imprimir(
            'R$', str(round(compra['valor'], 2)), tamanho=20, alinhar='fim')
        total += compra['valor']
    pr.separador(120, caracter='─')
    pr.imprimir('Total de vendas do caixa',
                tamanho=107, end='│', alinhar='fim')
    pr.imprimir('R$', str(round(total, 2)), tamanho=12,
                alinhar='fim', cor_texto='vermelho negrito')
...
def valor_total_pagar(compra):
    ...
    if (quantidade_codigo_10 >= 2):
        ...
        descontos.append({'descricao': '50% no segundo produto ' +
                         produto_10['nome'], 'valor': (produto_10['valor'] * 0.5)})

    if (total > 100):
        descontos.append(
            {'descricao': '10% de desconto na compra acima de R$ 100,00', 'valor': (total * 0.1)})
        total = total * (1 - 0.10)
